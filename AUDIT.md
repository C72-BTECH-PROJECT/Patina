...
`Upload.jsx` (pick job, choose PDF/DOCX — **no JD paste field**) → `Processing.jsx` `POST http://localhost:5000/api/analyze` (multipart: `resume`, `jobId`) → `analysis.controller.analyzeResume` looks up job in Supabase, forwards `resume` + `jd` (the job's markdown `description`, semantic text only) + `required_skills` + `preferred_skills` (the job's structured arrays, **authoritative job-skill source**) to `http://localhost:8000/parse` → FastAPI returns entities + `semantic_analysis` (now including `jd_skills`, `jd_source`, `resume_skills` and `component_weights`; skills normalised/deduped on both sides before intersecting) → controller uploads the raw file to the `resumes` storage bucket, inserts a `resumes` row (parsed entities) and a `scores` row built by `Services/explainability.buildScoreRecord` (composite, all three sub-scores in a uniform shape, redistributed weights, and the full explanation record incl. per-skill evidence) → `Processing.jsx` navigates to `/candidate/dashboard` (a static marketing page). **[Explainability 2026-09-03]** Recruiter `Dashboard.jsx`, `JobDetails.jsx` and `CandidateModal.jsx` all read `GET /api/candidates` (Supabase `scores` ⨝ `resumes` ⨝ `profiles`, scoped to the recruiter's own jobs, latest score per candidate) plus `GET /api/jobs/mine`; `CandidateModal` renders the persisted explanation verbatim via `components/recruiter/ScoreBreakdown.jsx`. No mock source remains on the recruiter path.
...
| 1.5 Custom/trained NER model | DONE_BUT_BROKEN_OR_HACKY | `ner.py` loads stock `en_core_web_sm`; skills still come from the hardcoded `SKILL_KEYWORDS` list + regex, but are now run through `normalize_skills` (canonical case + variants: `Node.js`/`nodejs`, `REST APIs`/`rest`/`api`, duplicate catalog entries all collapse to one token) on both the résumé and job side before intersecting. Experience / education / projects are **section-scoped** via a new `split_sections()`: experience is parsed only from a detected EXPERIENCE/EMPLOYMENT section (employer / role / dates / highlights) and is an **empty list when that section is absent** — the old "dump every spaCy ORG/DATE entity" block and `is_plausible_organisation` are deleted, so phone numbers tagged DATE and acronyms (`SVG`, `SSL`, `Google`) tagged ORG no longer appear as fake job history; project titles are gated to title patterns (`Title \| Tech`, or a short heading line immediately followed by bullets) so action-verb bullets ("Engineered…", "Trained…", "Deployed…") attach as `description` instead of becoming their own project; education is taken from the education section only (no more certification bullets leaking in). **[Parsing fixes 2026-09-03]** | Still no custom NER model, no HF "Annotated NER PDF Resumes" dataset, no training — classification approach unchanged, so this stays BROKEN_OR_HACKY. But the fabricated-experience / bullet-as-title / education-leak defects found in live testing are fixed: extraction is now honest (empty where the section is absent) rather than invented, and skill sets are deduped/normalised. |
...
| 1.14 Parsing sub-score emitted & persisted | DONE_BUT_BROKEN_OR_HACKY | `embedder.analyse` returns `similarity_score`, `skill_coverage`, `matched_skills`, `missing_skills`, `jd_skills`, `resume_skills`, `focused_score`, `full_text_score`, `match_reason`, `component_weights` and (**[Parsing fixes 2026-09-03]**) `jd_source` (`structured_skills` \| `parsed_description`). Job skills now come from the stored job's `required_skills` + `preferred_skills` arrays (clean, structured); the markdown `description` is scraped only as a fallback when both arrays are empty. `Services/explainability.buildParsingSubScore` carries all of it into `scores.subscores.parsing_semantic_alignment`, records `jd_source` in `components`, and names the source in the skill-coverage evidence note. | Nothing the NLP service computes is dropped by Node. Skill matching is now against authoritative structured data instead of a keyword scrape of markdown. Still the *only* input, so it equals the composite until GitHub/assessment land; underlying number is still keyword-coverage-dominated (see 1.7) — that keeps this at BROKEN_OR_HACKY. `preferred_skills` are currently matched with the same weight as `required_skills` (a distinct lower weight is a **future item** — no such data exists yet: the one job row has `preferred_skills = []`). |
...
| Item | Classification | Evidence | Notes |
|---|---|---|---|
| 5.1 Web upload portal (validation, progress, errors) | STUBBED_OR_PARTIAL | `Upload.jsx` — drag/drop, `.pdf,.docx` filter, job select, error messages. **[Candidate loop 2026-09-03]** `Processing.jsx` progress is now **real** — XHR `upload.onprogress` for the upload, an honest indeterminate state for the (un-streamed) server analyze, and a real error panel on non-2xx / network failure. **[Parsing fixes 2026-09-03]** the candidate-supplied JD paste field is **removed** from `Upload.jsx` — it was wrong by design (the candidate already selects a job that has a description, and a self-supplied JD is a self-scoring vector); the analyze path no longer carries a `jd` request param from the browser. | The fake random progress is fixed; the JD self-scoring hole is closed. Still open: the "Choose File" path (`handleFileChange`) doesn't validate type; no corrupt-file handling. |
...
- **[Persistence bug 2026-09-03] — ROOT CAUSE FOUND & FIXED.** Symptom: every resume upload fails at persistence — storage returns `AccessDenied` / "new row violates row-level security policy", the `resumes` insert returns Postgres `42501`. The `20260903_create_resumes_and_scores.sql` tables *are* applied and the service-role key *is* valid; the bug is in `auth.controller.js`. **`login` (and `signup`) called `supabase.auth.signInWithPassword` / `supabase.auth.signUp` on the shared service-role client** (`Config/supabase.js` singleton). supabase-js caches that session in memory, and from then on `fetchWithAuth` sends the **logged-in user's JWT** as `Authorization` on every `.from()` / `.storage` call on that singleton instead of the service-role key. It is process-global and sticky: the first login after a server restart breaks all writes until the next restart (nothing calls `signOut`). Reads mostly survived only because `resumes`/`scores` have SELECT policies; there are no INSERT policies, so writes always 42501. Reproduced exactly (create confirmed user → `signInWithPassword` on the shared client → `resumes` insert → `42501`; storage upload → `AccessDenied`). **Fix, two layers:** (1) `auth.controller.js` — `login`/`signup`/`resendConfirmation` now run every GoTrue call on a throwaway anon client (`createUserScopedClient()`, already used by the password-reset flow) and `signOut()` it; the shared client only ever does `auth.admin.*` and RLS-bypassing DB writes. (2) `Config/supabase.js` — the service-role key is pinned as `global.headers.Authorization`, so even a stray in-memory session can no longer downgrade REST/storage calls (verified: `signInWithPassword` directly on the shared client, then insert + upload, both succeed). Plus a one-line boot log: `[supabase] service client key role: service_role (project …)`.
- **[Persistence 2026-09-03] Storage MIME allow-list.** Separate, real, lower-severity: the `resumes` bucket was created by hand 2026-08-31 (not by the migration) with `allowed_mime_types` = pdf + docx only and a 10 MB cap. `analysis.controller.js` uploaded with `req.file.mimetype || 'application/octet-stream'`; when multer/the browser reports the part as `application/octet-stream` (common for DOCX) storage 415s. Currently non-fatal (file dropped, `file_ref` null, row still saved). Fixed: `resolveContentType()` derives the type from the file extension first. Also, every persistence failure branch now logs the full Supabase error (`code`/`details`/`hint` for PostgrestError, `status`/`statusCode` for StorageApiError) with an RLS-specific hint, instead of bare `.message`.
...
### [Parsing / JD-source fixes 2026-09-03] — bugs found in live testing

- **Three competing JD sources, no precedence — FIXED.** The `jobs` row has
  structured `required_skills` / `preferred_skills` arrays *and* a markdown
  `description`, and `Upload.jsx` also asked the candidate to paste their own JD;
  in testing the pasted JD was dropped and the markdown `description` was
  keyword-scraped for skills. Now: the stored job is the sole authority.
  `analysis.controller.js` forwards `required_skills` + `preferred_skills` (JSON)
  alongside `jd` (the markdown, used only as semantic-similarity text);
  `embedder._resolve_jd_skills` uses the structured arrays as the job-skill
  source and only falls back to parsing the markdown when both are empty.
  `jd_source` (`structured_skills` | `parsed_description`) is returned by the NLP
  service, persisted in `scores.subscores.parsing_semantic_alignment.components`
  and named in the skill-coverage evidence note. The JD paste field and the
  browser-side `jd` param are removed (`Upload.jsx`).
- **Skill sets not deduped/normalised — FIXED.** `ner.normalize_skill` /
  `normalize_skills` give one canonical token per skill (reverse of
  `SKILL_ALIASES` + an `_EXTRA_NORMALISATION` table: `rest`/`api`/`REST APIs`
  → `rest api`, `Node.js` → `nodejs`, `ci`/`cd` → `ci/cd`, etc.), applied to
  both the résumé and job side in `embedder.analyse` before intersecting. The
  duplicate `"typescript"` entry in `SKILL_KEYWORDS` is removed and
  `extract_skills` dedupes its own output.
- **Fabricated experience — FIXED.** The block that pushed every spaCy ORG/DATE
  entity in the whole document into `experience` (phone numbers as DATE;
  `EJS`/`SVG`/`SSL`/`ML`/`ETL`/`Google` as ORG → "Experience (19)") is deleted,
  along with `is_plausible_organisation`. `ner._extract_experience` now parses
  only a detected EXPERIENCE/EMPLOYMENT section (via `split_sections`), yielding
  `{title, organisation, dates, highlights}` entries, and returns `[]` when no
  such section exists. Entry shape change rippled to the one consumer,
  `frontend/src/pages/candidate/Results.jsx` (grep confirmed: recruiter
  `Dashboard`/`JobDetails`/`CandidateModal` render `job.experienceLevel`, never
  parsed experience internals).
- **Bullet points promoted to project titles — FIXED.** `ner._extract_projects`
  scopes to the projects section and only treats a line as a title if it matches
  a title pattern (`Title | Tech` / `Title – Tech` / `Title:`) or is a short
  heading-like line immediately followed by a bullet. Bullets and action-verb
  sentences ("Engineered…", "Trained…", "Deployed…") attach to the current
  project's `description`. Shape unchanged (`{title, description}`).
- **Education scope leak — FIXED.** `ner._extract_education` reads the education
  section when one exists (keeping entry lines and bare years); without one it
  falls back to a whole-document scan that requires a strong degree/institution
  keyword, so a certification bullet ("Mastered core Agile principles…") under a
  CERTIFICATIONS heading no longer appears under Education.
- **Future item (noted, not done):** a distinct, lower match weight for
  `preferred_skills` vs `required_skills`. Deferred because the only job row in
  the DB has `preferred_skills = []` — introducing an untested scoring weight
  for data that doesn't exist was out of scope. Preferred skills are currently
  matched as required skills.
- **Out of scope, noticed while editing these files:** `SkillsSelector` in
  `Upload.jsx` (hardcoded "Suggested Skills" chips, never wired —
  `selectedSkills`/`onToggle` props never passed; eslint `no-unused-vars`);
  `Upload.jsx` "Choose File" path still doesn't validate file type;
  `"exponentjs"` and similar apparent AI-cruft entries remain in
  `SKILL_KEYWORDS`.
