...
function Upload() {
  ...
  const handleAnalyze = async () => {
    ...
    navigate('/processing', { state: { file, jd, formData: { resume: file, jd } } });
  };
      ...
      <main className="flex justify-center items-center min-h-[calc(100vh-80px)] py-12 px-6">
        <div className="max-w-2xl w-full">
          ...
          <p className="text-lg text-slate-500 text-center mb-10">
            Upload your resume and paste a job description to start the skill verification process
          </p>
          ...
          <div className="mt-6 bg-white rounded-xl shadow-sm p-6">
            ...
            <p className="text-sm text-slate-500 mb-3">
              Paste the job description to compare your resume against
            </p>
            <textarea
              className="w-full h-40 p-4 border border-slate-200 rounded-lg text-slate-700 text-sm resize-none focus:outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500"
          ...
          <div className="mt-6 p-6 bg-white rounded-xl shadow-sm text-center">
          ...
          {error && (
            <div className="mt-4 p-4 bg-red-50 border border-red-200 rounded-lg text-red-600 text-sm text-center">
              {error}
            </div>
          )}
          ...
          <div className="mt-6 text-center">
            <button
              className={`group inline-flex items-center justify-center py-4 px-8 rounded-lg font-medium text-lg text-white transition-all ${
                file && jd.trim()
                  ? 'bg-indigo-600 hover:bg-indigo-700 cursor-pointer shadow-md shadow-indigo-500/30'
                  : 'bg-slate-400 cursor-not-allowed'
              }`}
              onClick={handleAnalyze}
              disabled={!file || !jd.trim() || isLoading}
            >
              {isLoading ? 'Preparing...' : 'Analyze Resume'}
              <span className={`ml-2 transition-transform ${file && jd.trim() ? 'group-hover:translate-x-1' : ''}`}>→</span>
            </button>
            {!jd.trim() && file && (
              <p className="text-sm text-amber-500 mt-2">Please add a job description to continue</p>
            )}
          </div>
        </div>
      </main>
